-- ============================================================
-- Migration 043: Memory Match game support
-- Adds new game type, personal best column, and updates RPCs.
-- ============================================================

-- ----------------------------------------------------------------
-- 1. Extend game_scores to accept memory_match
-- ----------------------------------------------------------------
...
ALTER TABLE game_scores
  ...
  CHECK (game_type IN ('speed_round', 'word_rain', 'memory_match'));

-- ----------------------------------------------------------------
-- 2. Extend user_game_stats with best_memory_match
-- ----------------------------------------------------------------
ALTER TABLE user_game_stats
  ADD COLUMN IF NOT EXISTS best_memory_match INTEGER NOT NULL DEFAULT 0
  CHECK (best_memory_match >= 0);

-- ----------------------------------------------------------------
-- 3. Update submit_game_score RPC
-- ----------------------------------------------------------------
...
BEGIN
  ...
  END IF;
  IF p_game_type NOT IN ('speed_round', 'word_rain', 'memory_match') THEN
...
-- ----------------------------------------------------------------
-- 4. Update get_game_leaderboard RPC
-- ----------------------------------------------------------------
